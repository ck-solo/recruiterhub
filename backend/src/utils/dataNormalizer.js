...
export function normalizeCompany(company) {  // "  Microsoft   Corporation, Inc.  "
  ...
  return cleanString(company) 
    ...
    .toLowerCase()
    .replace(
  /\b(co\s+ltd|pvt\s+ltd|corporation|limited|company|incorporated|llc|inc|corp|ltd|gmbh|pvt|co|sa|ag)\b/g,
  ""
)
    .replace(/\s+/g, " ")
    .trim();
...
export function parseEmploymentType(typeRaw) {
  ...
  if (/temporary/.test(raw)) return "Temporary";
if (/contract|freelance|consultant/.test(raw)) return "Contract";
...
export function parseSalary(salaryRaw) {
  ...
  .toUpperCase();
    ...
    if (clean.endsWith("M")) value *= 1000000;
  ...
  /(?:USD|INR|GBP|EUR|CAD|AUD|[$£€₹]|C\$|A\$)?\s*([\d,.]+(?:\s*[KM])?)\s*(?:-|TO)\s*(?:USD|INR|GBP|EUR|CAD|AUD|[$£€₹]|C\$|A\$)?\s*([\d,.]+(?:\s*[KM])?)/
);
...
if (rangeMatch) {
  ...
  const singleMatch = norm.match(
    /(?:USD|INR|GBP|EUR|CAD|AUD|[$£€₹]|C\$|A\$)?\s*([\d,.]+(?:\s*[KM])?)/
  );

  if (singleMatch) {
    min = max = cleanNum(singleMatch[1]);
  }
}
...
export function normalizeJobRow(row) {
  ...
  return {
    title,
    titleNormalized: cleanString(title)
  .replace(/[^\w\s]/g, "")
  .toLowerCase(),
    company,
